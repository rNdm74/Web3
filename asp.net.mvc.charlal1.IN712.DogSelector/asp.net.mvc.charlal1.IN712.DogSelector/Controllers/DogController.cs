...
namespace asp.net.mvc.charlal1.IN712.DogSelector.Controllers
{
    public class DogController : Controller
    {
        ...
        {
            // Build Dropdown box
            List<String> PreferredSizes = new List<String> { "Pocket", "Tiny", "Miniature", "Small", "Medium", "Large", "Giant" };
            List<SelectListItem> PreferredSizesList = new List<SelectListItem>();
            SelectListItem newItem;
            foreach (String s in PreferredSizes)
            {
                newItem = new SelectListItem { Text = s, Value = s };
                PreferredSizesList.Add(newItem);
            }

            ViewBag.Size = PreferredSizesList;
        ...
        public ActionResult ChooseDogForm(Dog dreamDog) 
        {
            ...
            List<Dog> dogDB = makeDatabase();

            int matches = 0;

            foreach (Dog dog in dogDB)
            {
                int newMatch = CountMatches(dreamDog, dog);
        ...
        private int CountMatches(Dog d1, Dog d2) 
        {
            ...
            if (d1.SheddingLevel.Equals(d2.SheddingLevel))
                matches++;

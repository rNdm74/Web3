...
var Painting = {
    redraw: function() {
        ...
        for (row = 0; row < imgs.length; row++) {
            var img = new Image;      // First create the image...
                img.onload = function() {  // ...then set the onload handler...
                    //ctx.drawImage(img, 0, row * TILE_HEIGHT, TILE_WIDTH, TILE_HEIGHT);
                    imageArray.push(img);
                };
                img.src = imgs[row].src;      // *then* set the .src and start it loading.
            
//            for (row = 0; row < 1; row++) {
//                //alert(imgs[i].src);
//                var img = new Image;      // First create the image...
//                img.onload = function() {  // ...then set the onload handler...
//                    ctx.drawImage(img, col * TILE_WIDTH, row * TILE_HEIGHT, TILE_WIDTH, TILE_HEIGHT);
//                };
//                img.src = imgs[count].src;      // *then* set the .src and start it loading.
//                
//                count++;
//            }
